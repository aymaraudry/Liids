...
export async function runEnrichmentBatch(batchSize = 5): Promise<{ processed: number; contactsFound: number; errors: string[] }> {
  ...
  for (const company of pending) {
    // Check if user requested stop between each company
    const { data: stopCheck } = await supabase
      .from('settings')
      .select('pipeline_stop_requested')
      .single()
    if (stopCheck?.pipeline_stop_requested) {
      console.log('[Enrichment] Stop requested by user — halting batch')
      // Clear the flag
      await supabase.from('settings')
        .update({ pipeline_stop_requested: false })
